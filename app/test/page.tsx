...
const allUsers = [
  ...
  { id: "4", name: "Alex Kumar", initials: "AK" },
  { id: "5", name: "Emily Davis", initials: "ED" },
  { id: "6", name: "Tom Wilson", initials: "TW" },
  { id: "7", name: "Lisa Park", initials: "LP" },
];
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            dummyIssues.map((issue) => {
              ...
              return (
                <Link key={issue.id} href={`/issues/${issue.issueNumber}`}>
                {(() => {
                    const statusConfig = getStatusConfig(issue.status);
                    const StatusIcon = statusConfig.icon;
                    ...
                    <div
                        className={cn(
                        "rounded-xl p-5 border transition-all cursor-pointer group",
                        "bg-[hsl(var(--card))] border-[hsl(var(--border))] hover:shadow-lg hover:border-[hsl(var(--primary))]"
                        )}
                    >
                        {/* ===== Line 1: IS-number — Title — Category  |  Status (+ Priority if not closed) ===== */}
                        <div
                        className={cn(
                        ...
                        <div className="flex items-baseline gap-2 min-w-0">
                            <span className="text-sm font-bold text-[hsl(var(--primary))] font-mono shrink-0 leading-none">
                            IS-{issue.issueNumber}
                            </span>

                            <span className="shrink-0 text-[hsl(var(--muted-foreground))]">—</span>

                            <h3 className="text-lg font-bold text-[hsl(var(--foreground))] group-hover:text-[hsl(var(--primary))] transition-colors truncate leading-tight">
                            {issue.title}
                            </h3>
                            ...
                            variant="outline"
                            className={cn(
                                "ml-1 px-2 py-0.5 text-xs font-semibold shrink-0",
                        ...
                        {/* ===== Line 2: Description (clipped) ===== */}
                        <p className="text-sm text-[hsl(var(--muted-foreground))] line-clamp-2 mt-2">
                        {issue.description}
                        </p>

                        {/* ===== Line 3: Footer (grey-out when closed) ===== */}
                        <div
                        className={cn(
                            "flex items-center justify-between gap-4 flex-wrap mt-4",
                            isClosed && "opacity-60"
                        )}
                        >
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex -space-x-2">
                                ...
                                <Avatar key={user.id} className="h-7 w-7 border-2 border-[hsl(var(--card))]">
                                ...
                                <div className="h-7 w-7 rounded-full border-2 border-[hsl(var(--card))] bg-[hsl(var(--muted))] flex items-center justify-center">
                            ...
                            {issue.imageCount > 0 && (
                                ...
                                <span className="font-medium">{issue.imageCount}</span>
                                </div>
                            )}
                            </div>

                            {/* Deadline (hide when closed) */}
                            {issue.deadline && !isClosed && (
                            <div
                                className={cn(
                                "flex items-center gap-1 px-2 py-1 rounded-md text-xs font-semibold",
                                issue.deadline < new Date() ? "bg-red-50 text-red-600" : "bg-blue-50 text-blue-600"
                                )}
                            >
